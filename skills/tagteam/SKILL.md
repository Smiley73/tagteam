...
The other half of that guarantee is in the workflow: a step that carries questions may return them or omit exactly the ones a human decision answered, and nothing else. Dropping one is the failure that looks like success — the sidecar shrinks, the pass reports fewer questions than it was given, and the decision the question stood for silently becomes an assumption. Both engines are checked, on round revisions and on the continuation that integrates answers; a fresh draft carries nothing, so there is nothing there to check.

Reviewers are read-only and the drafter that wrote the sidecar ran before them, so the questions a round's reviewers raise reach that file only by way of the revision that follows. A round that ends the loop has no such revision — a clean one and a divergent one both stop there — so the pass merges its reviewers' questions itself at every exit.
