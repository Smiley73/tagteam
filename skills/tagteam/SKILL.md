...
The carried set never travels as content. It reaches the merge as a path the plumbing command reads with its own filesystem access, alongside the decisions file whose rows retire part of it, and the workflow names the digest of the union it expects so the command cannot write anything else. Only what a read-only reviewer raised — which exists in no file, because no reviewer may write one — is passed inline, in batches small enough that no single argument approaches the per-argument size ceiling.
