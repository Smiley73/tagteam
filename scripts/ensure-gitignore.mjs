...
export const BEGIN = "# >>> tagteam managed — regenerated by /tagteam:configure; edits inside are lost >>>";
export const END = "# <<< tagteam managed <<<";
// Headers earlier versions wrote. The block is found by its header line, so a
// repository configured before the command was renamed still has to be
// recognised as managed — otherwise this would append a second block beneath
// the first and report the old header as a comment somebody wrote.
export const LEGACY_BEGINS = ["# >>> tagteam managed — regenerated by /tagteam:init; edits inside are lost >>>"];
const isBegin = (line) => line === BEGIN || LEGACY_BEGINS.includes(line);
...
//
// `codegraph`: `/tagteam:configure` runs `codegraph init` when the user opts in,
...
function orphanedComments(lines, removedIndexes) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    if (!isComment(lines[index]) || isBegin(lines[index]) || lines[index] === END) continue;
    let cursor = index + 1;
    const run = [];
