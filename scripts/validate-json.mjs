...
// The configuration version the current plugin writes. Version 9 adds the
// required `providers` map that chooses the engine for the two code-writing
// workers. No key in this project has a fallback in a script: the file is the
// whole configuration, so a missing key is a hard error rather than a
