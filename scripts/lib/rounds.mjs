...
// attempt is the implementation, not a fix, so the ship passes `exempt: 1`. That
// is policy, so it is stated by the caller and has no default here — a default
// would be policy hiding in a library.
//
...
//
// **The same attempt re-enters its round.** The ship knows an attempt by the
// commit it is reviewing, so a candidate that owns a round in the current scope
// gets that round back and spends nothing: pay once for work already paid for,
// and never twice. The plan side has no rounds at all any more — a plan is
// reviewed once and answered — so this module serves the ship alone.
...
function decideRound(resolved, { candidate, scope, limit, limitName, exempt }) {
  ...
  if (already) {
    // Nothing is emptied here: the ship clears its round at the snapshot step,
    // and clearing here as well would delete the snapshot it is about to re-enter.
 ...
 * spent.
 *
 * `candidate` is opaque and is not validated as a commit id. A round whose
 * record names this candidate in this scope is **re-entered** — the same round
 * comes back, marked, having spent nothing. That is the resume case, and it is
 * why an interrupted attempt picked up again does not pay a second time for a
 * round it already paid for.
 *
...
export async function allocateRound(root, options = {}) {
  const { candidate, scope, limit, limitName, exempt } = options;
...
// A command-line face, kept for the tests that race several real processes at
// one root: a budget checked concurrently is only a budget if the allocation
// lock holds across processes, and only processes can prove that.
const USAGE = `usage:
