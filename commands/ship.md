...
1. Parse `--resume`, `--dry-run`, and `--reviewers`. Reject a missing reviewer value; a named reviewer is force-enabled even if disabled/conditional; `all` forces every dimension.
...
4. Require the primary checkout to be clean at ship start. For a new ship, normalize the current dual-provider policy with `node "${CLAUDE_PLUGIN_ROOT}/scripts/lib/run-policy.mjs" normalize "both" "<repo>/.tagteam/config.json"` before model work. On resume, run `run-policy.mjs restore "<ship-dir>/run-policy.json" "<repo>/.tagteam/config.json"` and pass every saved ship/PR JSON artifact as a repeated `--state "<artifact>"` argument. The restore validates that all embedded fingerprints match. A missing policy is a hard stop whenever any saved state is policy-bound. Add `--allow-legacy` only after proving saved model/state artifacts exist and none contains a policy fingerprint; this performs the one-time validated pre-feature `both` migration at mode 0600. Re-check `codex --version`, `gh auth status` in GitHub mode, origin, and base protection. Snapshot the resolved base name and starting OID. Never infer that protection still exists from init.
