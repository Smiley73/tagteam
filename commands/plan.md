...
2. Read `goal.json` for the original goal and run overrides. Write it at the start of every invocation next to the drafts. Determine the highest `pass-<n>` from the saved draft/review artifacts before resolving policy or usage. Restore `reviews/<passId>-run-policy.json` with `run-policy.mjs restore`, supplying every saved JSON artifact for that pass with `--state`. If the policy is missing, stop when any state contains a policy fingerprint. Add `--allow-legacy` only after scanning the saved model/state artifacts and proving they exist but all predate policy fingerprints; this performs the one-time validated `both` migration.
