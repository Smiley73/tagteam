...
4. Re-invoke `tagteam:plan-forge` with the same arguments plus `passId` (the same pass), `seedPlan: { path: <that file's absolute path> }` (do not read or pass its contents; `seedPlanPath` is retained only for compatibility with saved recovery descriptors and older callers), `openQuestions` (that sidecar's array), `uiDecisions` (the interface sidecar's array, or `[]` when that file is absent or unreadable), `uiDecisionsFile`, persisted `agentCalls`, `usage`, `usageReceipts`, `usageAccounting`, and `resumeRound: <r>`. When the selected provider is `codex` and the interface sidecar is absent, empty, malformed, or unreadable, first atomically write `[]` to a mode-0600 `reviews/<passId>-recovered-ui-decisions.json` and pass that path as `uiDecisionsFile`; never point the workflow at the missing path. Otherwise pass the readable sidecar's absolute path. The workflow verifies and reads the plan by reference, skips drafting, restarts at round `r`, and reuses every saved Codex result whose recorded request matches. Never drop a saved question: an unanswered one is a decision the human still owes. A round input and its sidecar are published together, by the workflow, only once the checks on that revision have passed, so the newest one you find is a revision that cleared them — there is no round input written by a step that then failed. Plans drafted before that was true are the exception, and they are still on disk: a revision then wrote the round input itself, so a pass that stopped because a plan result dropped an unresolved carried question left the shortened sidecar at exactly the path this step selects. When the interruption that ended the previous pass says that, and no `reviews/<passId>-round-<r>-revision-work.md` exists beside the round input to show it was published rather than written in place, resume from the round before it instead.
...
Record each answer as a `{question, answer}` row whose `question` is the sidecar entry **verbatim**. The workflow matches decision rows to carried questions by normalized exact text, so a paraphrased row does not answer anything: the next step reads that question as still open and stops the pass for dropping it. The match-on-meaning rule above governs which questions are worth asking, never what gets written down.
