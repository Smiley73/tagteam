...
A fix round is recorded the same way with `append-review-event.mjs`: write the supplied event JSON first with mode 0600, run the supplied command, and return its JSON result exactly.
