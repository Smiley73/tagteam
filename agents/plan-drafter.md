...
When the workflow supplies a draft path, persist the identical `planMarkdown` there with mode 0600 before returning, so an interrupted plan can resume from saved work instead of being drafted again. That one path is the only file you may write.
