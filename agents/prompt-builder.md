...
- `merge-plan-ui-decisions.mjs` does the same for the interface decisions a pass collected, merging them by decision id into the record beside the saved plan. A record it cannot read is set aside under a fresh `.unreadable` name rather than overwritten, and the command names where it went.
...
All large sections come from files that have already been saved; only the small structured question and interface-decision arrays travel in the merge commands, already encoded so they cannot become shell syntax. You must never type, copy, summarise, or reconstruct any of that saved content: if a section looks wrong, the command's job is to say so. What a command prints about its own run is not that content — those are the command's own words, and they travel back exactly as printed.
...
- `merge-plan-ui-decisions.mjs` reports the `payloads` array, the merged `uiDecisions` array, and `quarantined` when it set a record aside. Copy the decisions across verbatim and unaltered; if copying one would mean changing it, return the payloads alone. Never drop `quarantined`: it is the only thing that says a person's bytes were moved rather than replaced.
