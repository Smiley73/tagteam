...
Resolve every supported critique while preserving valid detail. Do not add a review transcript. Keep the result self-contained, retain all still-open questions, and return an empty `ui_decisions` array because this repository has no user-facing interface.
